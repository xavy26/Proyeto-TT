...
func (s *SmartContract) CreateCandidate(ctx contractapi.TransactionContextInterface, candidateId string, position string, photo string, voterId string) error {
	// Validaciones de negocio
...
func (s *SmartContract) CreatePolitical_Party(ctx contractapi.TransactionContextInterface, political_partyId string, name string, description string, logo string, candidatesId []string) error {
	// Validaciones de negocio
	...
	for i := 0; i < len(candidatesId); i++ {
		fmt.Println(candidatesId[i])
		candidate, err := s.GetCandidateById(ctx, candidatesId[i])
		if err != nil {
			return fmt.Errorf("Error al obtener el candidato", err.Error())
		}
		candidates = append(candidates, candidate)
	}
...
func (s *SmartContract) CreateElection(ctx contractapi.TransactionContextInterface, electionId string, name string, description string, date_hour_start string, date_hour_end string, political_partiesId []string) error {
	// Validaciones de negocio
	...
	var political_parties []*Political_Party

	for i := 0; i < len(political_partiesId); i++ {
		fmt.Println(political_partiesId[i])
		political_party, err := s.GetPoliticalPartyById(ctx, political_partiesId[i])
		if err != nil {
			return fmt.Errorf("Error al obtener el partido político", err.Error())
		}
		political_parties = append(political_parties, political_party)
	}
...
func (s *SmartContract) CreateVote(ctx contractapi.TransactionContextInterface, voteId string, voterId string, political_partyId string, electionId string) error {
	// Validaciones de negocio
	...
	political_party, err := s.GetPoliticalPartyById(ctx, political_partyId)
	if err != nil {
		return fmt.Errorf("Error al obtener el partido politico", err.Error())
	}

	election, err := s.GetElectionById(ctx, electionId)
	if err != nil {
		return fmt.Errorf("Error al obtener la elección", err.Error())
	}
...
func (s *SmartContract) CreateResult(ctx contractapi.TransactionContextInterface, resultId string, nro_votes int, political_partyId string, electionId string) error {
	// Validaciones de negocio
...
func (s *SmartContract) CreateItem_Result(ctx contractapi.TransactionContextInterface, item_resultId string, nro_votes int, political_partyId string, resultId string) error {
	// Validaciones de negocio
